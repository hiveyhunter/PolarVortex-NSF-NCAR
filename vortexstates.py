# vtxplt.py - polar vortex analysis on ERA5 model-level data
...
try:
    ...
    raise RuntimeError(
        "cripser (CubicalRipser) is required by vtxplt; "
...
# Per-level trough cut. A displaced vortex can wrap and, at the threshold
# contour, MERGE with a separate lower-latitude trough basin (the boundary
# wraps the outside of both). The vortex is a basin (minimum) of phi; the
# trough is a separate, shallower basin joined only across a saddle. The
# per-level mask is segmented into phi-basins (markers = distinct minima).
# A non-vortex basin is CUT (dropped, severing the boundary at the saddle)
# only when it is BOTH:
#   (a) a real separate feature -- its prominence (depth of its minimum below
#       the saddle/col joining it to the vortex) is at least
#       BASIN_REAL_PROM_FRAC of the vortex's depth below the contour; this
#       guards a clean single vortex from being trimmed by a spurious dimple;
#   and (b) much shallower than the vortex -- its minimum sits at least
#       BASIN_TROUGH_GAP_FRAC of the vortex depth ABOVE the vortex minimum;
#       a genuine split lobe is deep (small gap) and is kept.
# BASIN_TROUGH_GAP_FRAC is the main knob: LOWER it to cut more aggressively
# (treat shallower second basins as troughs). Set it >= 1.0 to disable.
...
def analyze_geopotential(phi, altitude_km, percentile=20,
    ...
    # a component that sits on the equatorward (40 N) truncation edge and is
    # elongated ALONG it (wide in lon, shallow in lat) is truncation material,
    # not vortex; high aspect ratio away from the edge is left alone
    EDGE_ALONG_AR_MAX = 5.0

    def is_edge_truncation(m_cc):
        rows = np.where(m_cc.any(axis=1))[0]
        if rows.size == 0 or int(rows.min()) > 0:
            return False
        # latitude depth across the edge, longitude extent along it (periodic)
        ...
        return (lon_km / max(lat_km, 1e-6)) >= EDGE_ALONG_AR_MAX
    ...
    for k in range(nz):
        ...
        # (5) drop edge-truncation components: those touching the 40 N edge
        # and elongated along it (high lon/lat aspect ratio there). filaments
        # and deep lobes touching the edge are kept
        if np.any(bm):
